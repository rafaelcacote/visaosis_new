...
<tr>
    ...
    <td class="list-actions-col-descricao">
        ...
        <small class="text-muted d-md-none d-block mt-1">
            <i class="mdi mdi-map-marker me-1" style="font-size: 14px;"></i>
            @if ($locations->isEmpty())
                Nenhuma localização
            @else
                {{ $locations->map(fn($location) => $location->short_name ?? $location->name)->implode(', ') }}
            @endif
        </small>
    </td>
    ...
    <td class="list-actions-col-acoes">
        <div class="actions list-actions-buttons">
            <a href="{{ route('users.show', $user->id) }}" class="btn-action btn-action-view" title="Visualizar">
                <i class="mdi mdi-eye"></i>
            </a>
            <a href="{{ route('users.edit', $user->id) }}" class="btn-action btn-action-edit" title="Editar">
                <i class="mdi mdi-pencil"></i>
            </a>
            <button type="button" class="btn-action btn-action-password" title="Alterar Senha" data-bs-toggle="modal"
                data-bs-target="#changePasswordModal" data-user-id="{{ $user->id }}"
                data-user-name="{{ $user->name }}">
                <i class="mdi mdi-key"></i>
            </button>
            <form method="POST" action="{{ route('users.destroy', $user->id) }}" class="d-inline delete-user-form"
                ...
                <button type="button" class="btn-action btn-action-delete" title="Excluir" data-bs-toggle="modal"
                    ...
                    data-user-name="{{ $user->name }}" data-form-id="delete-form-{{ $user->id }}">
