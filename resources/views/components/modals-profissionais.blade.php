{{-- Modais da página de profissionais: desativar/ativar e excluir --}}

<!-- Modal de Confirmação de Desativar/Ativar Profissional -->
        ...
        <div class="modal-content">
            ...
            <div class="modal-body text-center px-4 pb-4">
                ...
                <p class="text-muted mb-0">
                    Tem certeza que deseja <span id="toggleStatusActionText">desativar</span> o profissional <strong id="toggleStatusProfName"></strong>?
                </p>
                <p class="text-muted mt-2 mb-0" style="font-size: 0.875rem;" id="toggleStatusHint">
                    O profissional não poderá ser utilizado em novos atendimentos até ser reativado.
                </p>
                <input type="hidden" id="toggleStatusProfId" value="">
...
<div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body text-center px-4 pb-4">
                <div class="mb-3">
                    <div class="delete-icon-wrapper delete-icon-wrapper-profissional">
                        <i class="mdi mdi-alert-circle-outline"></i>
                    </div>
                </div>
                <h5 class="modal-title mb-3" id="deleteModalLabel">Confirmar Exclusão</h5>
                <p class="text-muted mb-0">
                    Tem certeza que deseja excluir o profissional <strong id="deleteProfName"></strong>?
                </p>
...
@push('styles')
<style>
/* Modal Desativar/Ativar Profissional */
...
.delete-icon-wrapper-profissional {
    width: 80px;
    height: 80px;
    margin: 0 auto;
    display: flex;
...
.delete-icon-wrapper-profissional i {
    font-size: 48px;
    color: #dc2626;
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (deleteModal) {
        deleteModal.addEventListener('show.bs.modal', function(event) {
            ...
            const profissionalId = button.getAttribute('data-profissional-id');
            const profissionalNome = button.getAttribute('data-profissional-nome');
            const form = deleteModal.querySelector('#deleteForm');
            
            if (form) {
                form.action = `/profissionais/${profissionalId}`;
            }
            const nameElement = deleteModal.querySelector('#deleteProfName');
            if (nameElement) {
                nameElement.textContent = profissionalNome;
            }
        });
    }

    // Modal de Desativar/Ativar Profissional
    ...
    if (toggleStatusModal) {
        toggleStatusModal.addEventListener('show.bs.modal', function(event) {
            ...
            const nome = button.getAttribute('data-profissional-nome');

            document.getElementById('toggleStatusProfId').value = profissionalId;
            document.getElementById('toggleStatusNovoStatus').value = novoStatus ? 'true' : 'false';
            document.getElementById('toggleStatusProfName').textContent = nome;
            ...
            if (novoStatus) {
                ...
                hint.textContent = 'O profissional voltará a estar disponível para atendimentos.';
                hint.style.display = 'block';
                confirmBtn.className = 'btn btn-success';
                confirmBtn.innerHTML = '<i class="mdi mdi-check-circle me-2"></i> Ativar Profissional';
            } else {
                ...
                hint.textContent = 'O profissional não poderá ser utilizado em novos atendimentos até ser reativado.';
                hint.style.display = 'block';
                confirmBtn.className = 'btn btn-warning';
                confirmBtn.innerHTML = '<i class="mdi mdi-close-circle me-2"></i> Desativar Profissional';
            }
        });
    }
    ...
    if (toggleStatusConfirmBtn) {
        toggleStatusConfirmBtn.addEventListener('click', function() {
            const profissionalId = document.getElementById('toggleStatusProfId').value;
            const btn = this;
            ...
            fetch(`/profissionais/${profissionalId}/toggle-status`, {
                method: 'POST',
                headers: {
