    ...
    <div class="col-12">
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-6">
                        <input type="text" class="form-control" id="searchPatient" value="{{ old('search_term', '') }}" placeholder="Digite o nome, CPF ou telefone do paciente">
                    </div>
                    <div class="col-md-2">
                        <button type="button" class="btn btn-primary w-100" onclick="searchPatient(event)">
        ...
        <form id="triageForm" action="{{ route('attendance.triage.store') }}" method="POST">
            @csrf
            <!-- Patient Form -->
            <div class="card mb-4" id="patientForm" style="display: {{ $errors->any() || old('nome') || old('cpf') || old('profissional_id') || old('tipo') || old('prioridade') ? 'block' : 'none' }};">
                <div class="card-header">
                    <h5 class="mb-0">
                ...
                <div class="card-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="text" class="form-control @error('cpf') is-invalid @enderror" name="cpf" value="{{ old('cpf') }}" placeholder="000.000.000-00" maxlength="14">
                            @error('cpf')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-3">
                            ...
                            <input type="date" class="form-control @error('nascimento_em') is-invalid @enderror" name="nascimento_em" value="{{ old('nascimento_em') }}" required>
                            @error('nascimento_em')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-4">
                            ...
                            <input type="text" class="form-control @error('telefone') is-invalid @enderror" name="telefone" value="{{ old('telefone') }}" placeholder="(00) 00000-0000">
                            @error('telefone')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
            ...
            <!-- Triage Info (profissionais carregados no servidor; exibido após paciente escolhido/preenchido) -->
            <div class="card mb-4" id="triageInfo" style="display: {{ $errors->any() || old('nome') || old('cpf') || old('profissional_id') || old('tipo') || old('prioridade') ? 'block' : 'none' }};">
                <div class="card-header">
                    <h5 class="mb-0">
                ...
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md-3">
                            ...
                            <select class="form-select @error('tipo') is-invalid @enderror" name="tipo" required>
                                ...
                                <option value="{{ \App\Models\Consulta::TIPO_CONSULTA }}" {{ old('tipo') == \App\Models\Consulta::TIPO_CONSULTA ? 'selected' : '' }}>Consulta</option>
                                <option value="{{ \App\Models\Consulta::TIPO_RETORNO }}" {{ old('tipo') == \App\Models\Consulta::TIPO_RETORNO ? 'selected' : '' }}>Retorno</option>
                                <option value="{{ \App\Models\Consulta::TIPO_CONFERENCIA }}" {{ old('tipo') == \App\Models\Consulta::TIPO_CONFERENCIA ? 'selected' : '' }}>Conferência</option>
                            </select>
                            @error('tipo')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-3">
                            ...
                            <select class="form-select @error('prioridade') is-invalid @enderror" name="prioridade" required>
                                <option value="">Selecione...</option>
                                <option value="{{ \App\Models\Consulta::PRIORIDADE_NORMAL }}" {{ old('prioridade') == \App\Models\Consulta::PRIORIDADE_NORMAL ? 'selected' : '' }}>Normal</option>
                                <option value="{{ \App\Models\Consulta::PRIORIDADE }}" {{ old('prioridade') == \App\Models\Consulta::PRIORIDADE ? 'selected' : '' }}>Prioridade</option>
                                <option value="{{ \App\Models\Consulta::PRIORIDADE_EMERGENCIA }}" {{ old('prioridade') == \App\Models\Consulta::PRIORIDADE_EMERGENCIA ? 'selected' : '' }}>Emergência</option>
                            </select>
                            @error('prioridade')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('profissional_id') is-invalid @enderror" name="profissional_id" required>
                                <option value="">Selecione...</option>
                        ...
                        <div class="col-12">
                            ...
                            <textarea class="form-control @error('observacoes') is-invalid @enderror" name="observacoes" rows="3" placeholder="Observações adicionais sobre o atendimento...">{{ old('observacoes') }}</textarea>
                            @error('observacoes')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="mt-4">
                        <button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#cancelTriageModal">
...
<div class="modal fade" id="cancelTriageModal" tabindex="-1" aria-labelledby="cancelTriageModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body text-center px-4 pb-4">
                ...
                <p class="text-muted mb-0">Tem certeza que deseja cancelar a triagem?</p>
                <p class="text-muted mt-2 mb-0" style="font-size: 0.875rem;">Os dados inseridos serão perdidos e não poderão ser recuperados.</p>
            </div>
...
<script>
    window.searchUrl = '{{ route('attendance.patients.search') }}';
    window.indexUrl = '{{ route('attendance.index') }}';
    window.hasErrors = {{ $errors->any() ? 'true' : 'false' }};
    window.hasOldData = {{ old('nome') || old('cpf') || old('profissional_id') || old('tipo') || old('prioridade') ? 'true' : 'false' }};
    ...
    function searchPatient(event) {
        ...
        const button = event && event.target ? (event.target.tagName === 'BUTTON' ? event.target : event.target.closest('button')) : document.querySelector('button[onclick*="searchPatient"]');
        ...
        fetch(url, {
            ...
            .then(data => {
                if (data.found) {
                    if (data.multiple) {
                        ...
                        showNotification(`${data.count} pacientes encontrados. Selecione o paciente desejado.`, 'info');
                    } else {
            ...
            .catch(() => {
                hideAllForms();
                showNotification('Erro ao buscar paciente.', 'error');
            })
            .finally(() => {
                if (button) {
        ...
        pacientes.forEach(function(paciente) {
            ...
            listItem.innerHTML = `
                <div class="d-flex justify-content-between align-items-start">
                    ...
                    <small class="text-primary"><i class="mdi mdi-chevron-right"></i></small>
                </div>
            `;
    ...
    function showFormWithOldData() {
        ...
        if (window.hasOldName && !window.hasErrors) {
            const form = document.getElementById('triageForm');
            if (form) {
                form.querySelectorAll('input[name="nome"], input[name="cpf"], input[name="nascimento_em"], input[name="telefone"], input[name="email"]').forEach(input => {
                    input.readOnly = true;
                    input.classList.add('bg-light');
                });
            }
    ...
    function showNotification(message, type) {
        ...
        notification.style.cssText = `
            ...
            padding: 15px 20px; border-radius: 5px; z-index: 9999;
