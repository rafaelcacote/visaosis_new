                ...
                <div class="d-flex gap-2">
                    ...
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end">
                            ...
                            <li>
                                <a class="dropdown-item" href="#" onclick="filterByStatus('{{ \App\Models\Consulta::STATUS_AGUARDANDO }}')">
                                    Aguardando
                                </a>
                            </li>
                            <li>
                                <a class="dropdown-item" href="#" onclick="filterByStatus('{{ \App\Models\Consulta::STATUS_EM_ATENDIMENTO }}')">
                                    Em Atendimento
                                </a>
                            </li>
                            <li>
                                <a class="dropdown-item" href="#" onclick="filterByStatus('{{ \App\Models\Consulta::STATUS_ATENDIDO }}')">
                                    Atendidos
                                </a>
                            </li>
                            <li>
                                <a class="dropdown-item" href="#" onclick="filterByStatus('{{ \App\Models\Consulta::STATUS_CANCELADO }}')">
                                    Cancelados
                                </a>
                            </li>
                        </ul>
                        ...
                        <table class="table table-hover">
                            ...
                            <tbody id="consultasQueue">
                                @foreach ($consultas as $consulta)
                                    <tr data-status="{{ $consulta->status }}" data-priority="{{ $consulta->prioridade }}">
                                        <td>
                                            <div class="d-flex align-items-center">
                                                <div class="{{ $consulta->prioridade == \App\Models\Consulta::PRIORIDADE_EMERGENCIA ? 'bg-danger' : ($consulta->prioridade == \App\Models\Consulta::PRIORIDADE ? 'bg-warning' : 'bg-primary') }} text-white rounded-circle d-inline-flex align-items-center justify-content-center me-3" style="width: 40px; height: 40px; font-size: 16px; font-weight: 600;">
                                                    {{ strtoupper(mb_substr($consulta->paciente->nome ?? 'N/A', 0, 1, 'UTF-8')) }}
                                                </div>
                                                <div>
                                                    <div class="font-weight-medium">{{ $consulta->paciente->nome ?? 'Paciente não encontrado' }}</div>
                                                    @if ($consulta->prioridade == \App\Models\Consulta::PRIORIDADE_EMERGENCIA)
                                                        <small class="text-danger d-block mt-1">
                                                            <i class="mdi mdi-alert me-1" style="font-size: 14px;"></i>
                                                            Emergência
                                                        </small>
                                        ...
                                        <td>
                                            <div class="actions">
                                                ...
                                                @elseif($consulta->status == \App\Models\Consulta::STATUS_EM_ATENDIMENTO)
                                                    <button
                                                        class="btn-action btn-action-status-ativar"
                                                ...
                                                <button
                                                    ...
                                                    onclick="viewConsulta({{ $consulta->id }})"
                                                    title="Visualizar"
                                                >
                                                    <i class="mdi mdi-eye"></i>
                                                </button>
            ...
            <div class="card-body">
                @forelse ($profissionais as $profissional)
                    @php
                        ...
                        $emAtendimento = $consultas->where('profissional_id', $profissional->id)->where('status', \App\Models\Consulta::STATUS_EM_ATENDIMENTO)->first();
                    @endphp
                    <div class="d-flex align-items-center mb-3 p-3 border rounded">
                        <div class="avatar-circle {{ $emAtendimento ? 'bg-warning' : 'bg-success' }} text-white me-3">
                            {{ substr($profissional->nome, 0, 1) }}
                        </div>
                        <div class="flex-grow-1">
                            <h6 class="mb-1">{{ $profissional->nome }}</h6>
                            <small class="text-muted">{{ $profissional->especialidade->descricao ?? 'Especialidade não informada' }}</small>
                            @if ($emAtendimento)
                                <div class="mt-1">
...
<script>
    ...
    function openStatusModal(consultaId, pacienteNome, action) {
        ...
        if (action === 'iniciar') {
            ...
            alertText.innerHTML = `Tem certeza que deseja finalizar o atendimento para <strong>${pacienteNome}</strong>?`;
            modalDescription.textContent = 'O status da consulta será alterado para "Atendido" e o horário de finalização será registrado.';
            ...
            modalDescription.textContent = 'Esta ação não pode ser desfeita. O status da consulta será alterado para "Cancelado".';
