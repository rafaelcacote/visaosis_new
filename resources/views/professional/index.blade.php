...
@section('content')
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-flex gap-2 align-items-center">
                        ...
                        <span class="tag" style="background-color: #e0f0ff; color: #1d7dd6;">{{ $patients->count() }}
                            pacientes</span>
                    </div>
                </div>
                                        ...
                                        <tr data-status="{{ $patient->status }}">
                                            ...
                                            <td>
                                                ...
                                                @if ($patient->chegada_em || $patient->agendado_em)
                                                    @php
                                                        $horaReferencia = $patient->chegada_em
                                                            ? \Carbon\Carbon::parse($patient->chegada_em)
                                                            : \Carbon\Carbon::parse($patient->agendado_em);
                                                        $tempoEspera = $horaReferencia->diffForHumans(
                                                            \Carbon\Carbon::now(),
                                                            true,
                                                        );
                                                    ...
                                                    <small class="text-muted">
                                                        ...
                                                        @else
                                                            {{ floor($tempoEsperaMinutos / 60) }}h
                                                            {{ $tempoEsperaMinutos % 60 }}min
                                                        @endif
                                                    </small>
                                                @endif
                                            </td>
                                            ...
                                            <td>
                                                <div class="btn-group btn-group-sm">
                                                    ...
                                                    @elseif($patient->status == \App\Models\Consulta::STATUS_EM_ATENDIMENTO)
                                                        <a href="{{ route('professional.consultation', $patient->id) }}"
                                                            class="btn btn-sm btn-info" title="Continuar Atendimento">
                                                            <i class="mdi mdi-arrow-right"></i>
                                                        </a>
                                                    @endif
                                                    <button class="btn btn-outline-secondary" title="Histórico"
                                                        onclick="window.location.href='{{ route('professional.patientHistoryFull', $patient->paciente->id) }}?return_to=queue'">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    @forelse ($profissionais as $profissional)
                        <div class="mb-3 pb-2 border-bottom">
                            <div class="d-flex justify-content-between">
                                ...
                                @elseif(
                                    $profissional &&
                                        !$patients->where('profissional_id', $profissional->id)->where('status', \App\Models\Consulta::STATUS_EM_ATENDIMENTO)->count())
                                    <span class="tag tag-status tag-status-ativo">Disponível</span>
                                @else
                                    <span class="tag" style="background-color: #e0f0ff; color: #1d7dd6;">Em
                                        atendimento</span>
                                @endif
                            </div>
                            @if ($profissional)
                    ...
                    <div class="d-grid gap-2">
                        <button class="btn btn-outline-primary" onclick="openSelectProfissionalModal()">
    ...
    <div class="modal fade" id="startConsultationModal" tabindex="-1" aria-labelledby="startConsultationModalLabel"
        aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="alert alert-info">
                        ...
                        Tem certeza que deseja iniciar o atendimento para <strong
                ...
                <div class="modal-footer border-0 pt-0 justify-content-center">
                    <button type="button" class="btn btn-light" data-bs-dismiss="modal">
                        ...
                        Fechar
                    </button> <button type="button" class="btn btn-primary" id="confirmStartConsultationBtn">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div id="patientSearchResults" class="list-group shadow-sm"
    ...
    <div class="modal fade" id="selectProfissionalModal" tabindex="-1" aria-labelledby="selectProfissionalModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header border-0 pb-0">
                    <i class="bi bi-check-square me-2" id="modalIcon"></i>
                    <h5 class="modal-title" id="selectProfissionalModalLabel">Selecionar Profissional</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button>
                </div>
                <div class="modal-body">
                    <p>Selecione o profissional para pausar ou retomar atendimento.</p>
                    <form id="selectProfissionalForm">
                        <div class="mb-3">
                            <label for="profissionalSelect" class="form-label">Profissional</label>
                            <select class="form-select" id="profissionalSelect" name="profissional_id">
                                <option value="">Selecione...</option>
                                @foreach ($profissionais as $profissional)
                                    <option value="{{ $profissional->id }}"
                                        data-pausar="{{ $profissional->pausar_atendimento ? '1' : '0' }}">
                ...
                <div class="modal-footer border-0 pt-0 justify-content-center">
                    ...
                    <button type="button" class="btn btn-primary" onclick="confirmSelectProfissional()">
    ...
    <!-- Modal de Pausa de Atendimento -->
    <div class="modal fade" id="pauseModal" tabindex="-1" aria-labelledby="pauseModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header border-0 pb-0">
                    <h5 class="modal-title" id="pauseModalLabel">Pausar Atendimento</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button>
                </div>
                <div class="modal-body">
                    <div id="pauseModalText"></div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                    <button type="button" class="btn btn-primary" id="confirmPauseBtn">Confirmar</button>
                </div>
            </div>
        </div>
    </div>
...
@push('scripts')
    <script>
                            ...
                            .then(data => {
                                ...
                                if (data.found && data.pacientes.length > 0) {
                                    data.pacientes.forEach(patient => {
                                        ...
                                        const baseUrl =
                                            "{{ route('professional.patientHistoryFull', 'PLACEHOLDER') }}";
                                        item.href = baseUrl.replace('PLACEHOLDER',
                                            patient.id) + "?return_to=queue";
                                        item.className =
                                    ...
                                    resultsContainer.innerHTML =
                                        '<div class="list-group-item text-muted">Nenhum paciente encontrado</div>';
                                    resultsContainer.style.display = 'block';
                                }
                            })
                            .catch(error => {
                                ...
                                resultsContainer.innerHTML =
                                    '<div class="list-group-item text-danger">Erro ao buscar pacientes</div>';
                                resultsContainer.style.display = 'block';
                            });
                    }, 500);
                });
        ...
        function openSelectProfissionalModal() {
            const modal = new bootstrap.Modal(document.getElementById('selectProfissionalModal'));
            modal.show();
        }

        function confirmSelectProfissional() {
            const select = document.getElementById('profissionalSelect');
            const profissionalId = select.value;
            ...
            fetch(`/professional/toggle-pause/${profissionalId}`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
